<x-layout>
    <x-main-with-box>
        <x-breadcrumbs class="my-4"
            :links="['Admin Panel' => route('admin.index'), 'Movies Management' => route('admin.movies.index'), $movie->title => route('admin.movies.edit', $movie), 'Schedules' => route('admin.movies.schedules.index', $movie), 'Edit type' => '#']" />

        <x-title>Edit type for {{ $movie->title }}</x-title>
        <div x-data="{ loading: false }" class="flex justify-center">
            <form action="{{ route('admin.movies.types.update', ['movie' => $movie, 'type' => $type]) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4 w-80">
                ...
                <div class="mb-6 w-80">
                    ...
                    <x-text-input
                        name="language"
                        value="{{ old('language', $type->language) }}"
                    />
                </div>
