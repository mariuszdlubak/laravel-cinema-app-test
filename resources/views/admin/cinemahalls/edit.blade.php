    ...
    <x-main>
        <x-main-box>
            <x-breadcrumbs class="my-4"
                :links="['Admin Panel' => route('admin.index'), 'Cinema Halls Management' => route('admin.cinemahalls.index'), $cinemaHall->id => '#', 'Edit hall' => '#']" />

            <x-title>Cinema hall {{ $cinemaHall->name }}</x-title>
            <div x-data="{ isSelected: [], loading: false }" class="pt-10 flex flex-col gap-1 items-center">
                <form action="{{ route('admin.cinemahalls.update', $cinemaHall) }}" method="POST" class="flex flex-col items-center">
                @csrf
                @method('PUT')
                        ...
                        <div class="pt-2 pb-1 flex items-center gap-1 border-b border-b-text-200">
                            ...
                            @foreach ($seats as $s => $seat)
                                <div class="w-6 md:w-10 h-6 md:h-10">
                                    ...
                                    >
                                        @if(in_array($row . $seat, $blockedSeats))
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                fill="none" viewBox="0 0 24 24"
                                                stroke-width="1"
                                                :stroke="isSelected[{{ $r + 1 }}_{{ $s }}] ? '#e0e0e0' : '#292e3b'"
                ...
                <x-loading :x-show="'loading'">Updating the cinema hall is in progress</x-loading>
            </div>
        </x-main-box>
    </x-main>
</x-layout>
